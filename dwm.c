...
drawbar(Monitor *m)
{
	...
	drw_rect(drw, x + ulinepad, bh - ulinestroke - ulinevoffset,
		(ulinepad * 2) > w ? 0 : w - (ulinepad * 2), ulinestroke, 1, 0);
        x += w;
    }
        if (m->lt[m->sellt]->arrange == monocle) {
        for (c = nexttiled(m->clients), a = 0, s = 0;
             c;
             c = nexttiled(c->next), a++)
            if (c == m->stack)
                s = a + 1;
...
setup(void)
{
	...
	unsigned int alphas[] = {OPAQUE, baralpha, borderalpha, OPAQUE};
	for (i = 0; i < LENGTH(colors); i++)
		scheme[i] = drw_scm_create(drw, colors[i], alphas, 4);

	tagscheme = ecalloc(LENGTH(tagsel), sizeof(Clr *));
	for (i = 0; i < LENGTH(tagsel); i++)
		tagscheme[i] = drw_scm_create(drw, tagsel[i], alphas, 2);
